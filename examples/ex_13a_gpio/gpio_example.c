...
int gpio_example(void)
{
    ...
    while(1)
    {
        ...
        dwt_or16bitoffsetreg(GPIO_OUT_ID, 0, (GPIO_OUT_GOP3_BIT_MASK | GPIO_OUT_GOP2_BIT_MASK));
        Sleep(blink_delay);
        /* set GPIO2 & GPIO3 low (LEDs will be turned off) */
        /* Clear bits 2,3 */
        dwt_and16bitoffsetreg(GPIO_OUT_ID, 0, (uint16_t)(~(GPIO_OUT_GOP3_BIT_MASK | GPIO_OUT_GOP2_BIT_MASK)));
        Sleep(blink_delay);
    }
}
