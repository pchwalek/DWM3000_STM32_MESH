 ...
 */
static const dwt_aes_key_t aes_key = {0x41424344, 0x45464748, 0x49505152, 0x53545556,
        0x00000000, 0x00000000, 0x00000000, 0x00000000}; /*Initialize 128bits key*/
...
int simple_tx_aes(void)
{
    ...
    while(TRUE)
    {
        ...
        //memcpy(header.nonce, &nonce, sizeof(header.nonce));   /* embed low 6 bytes of the nonce as a GCMP HEAD as per 802.15.8 */
        status = dwt_do_aes(&aes_job,aes_config.aes_core_type);/* Note, 802.15.8 adds a MIC size of 16 bytes after payload */
        if (status<0)
