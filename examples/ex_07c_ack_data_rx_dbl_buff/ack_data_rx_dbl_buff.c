...
#if defined(TEST_ACK_DATA_RX_DBL_BUFF)
...
void read_received_data(uint8_t *rx_buff,buff_types_e buff_types)
{
    ...
    {   /* INDIRECT POINTER B is set up as part of DB configure to point to the buffer B - BUF1_FINFO. This is done in order to save time. */
...
int ack_data_rx_dbl_buff(void)
{
    ...
    while (1)
    {
        ...
        dwt_write8bitoffsetreg(RDB_STATUS_ID, 0, RDB_STATUS_CLEAR_BUFF0_EVENTS);//Clear status bits
        read_received_data(rx_buffer_a,buff_types_A);
        wait_for_ACK_TX_end_if_needed(rx_buffer_a[FRAME_CTRL_FIRST_BYTE_IDX]);
        dwt_signal_rx_buff_free();//Release the received buffer
        ...
        dwt_write8bitoffsetreg(RDB_STATUS_ID, 0, RDB_STATUS_CLEAR_BUFF1_EVENTS);//Clear status bits
